...
describe("last_seen_binary_version (v0.2.3)", () => {
  test("init writes scaffold_day_version + last_seen_binary_version equal to current", async () => {
    ...
    expect(file.scaffold_day_version).toBe("0.4.3");
    expect(file.last_seen_binary_version).toBe("0.4.3");
  });
  ...
  test("subsequent commands update last_seen_binary_version when stale", async () => {
    ...
    expect(after.last_seen_binary_version).toBe("0.4.3"); // refreshed
  });
  ...
  test("doctor surfaces both fingerprints", async () => {
    ...
    expect(r.stdout).toContain("initialized by: scaffold-day v0.4.3");
    expect(r.stdout).toContain("last seen by:");
    expect(r.stdout).toContain("current binary: scaffold-day v0.4.3");
  });
